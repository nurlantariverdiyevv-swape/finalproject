  ...
  return (
    <div className="w-full min-h-[calc(100vh-64px)] flex flex-col items-center justify-center text-center px-4 py-20 bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-center gap-2 mb-6 select-none" aria-hidden="true">
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <span key={i} className="w-1.5 h-1.5 rounded-full bg-gray-300" style={{ opacity: 1 - i * 0.15 }} />
        ))}
      </div>

      <Compass className="w-12 h-12 sm:w-14 sm:h-14 text-black mb-4 animate-[spin_9s_linear_infinite]" strokeWidth={1.25} aria-hidden="true" />
      ...
      <Link to="/" className="inline-flex items-center gap-2 bg-black text-white font-bold text-sm uppercase px-8 py-3.5 rounded-full hover:bg-gray-800 active:scale-95 transition-all duration-200">
        <ArrowLeft size={16} />
        Go back home
      </Link>
    </div>
